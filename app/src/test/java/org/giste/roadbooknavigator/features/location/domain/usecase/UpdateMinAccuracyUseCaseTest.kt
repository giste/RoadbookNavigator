...
class UpdateMinAccuracyUseCaseTest {

    private val repository: LocationSettingsRepository = mockk()
    private val useCase = UpdateMinAccuracyUseCase(repository)

    @Test
    fun `invoke should call repository when accuracy is valid`() = runTest {
        // Given
        val accuracy = 20.0f
        coEvery { repository.updateMinAccuracy(accuracy) } returns Unit
        ...
        coVerify { repository.updateMinAccuracy(accuracy) }
    }
    ...
    fun `invoke should return failure when accuracy is invalid`() = runTest {
        ...
        coVerify(exactly = 0) { repository.updateMinAccuracy(any()) }
    }
}
