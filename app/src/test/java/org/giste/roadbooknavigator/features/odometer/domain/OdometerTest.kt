...
class OdometerTest {
    ...
    fun `initialization clamps negative values to zero`() {
        ...
        Assert.assertEquals(0.0, odometer.total, 0.0)
        Assert.assertEquals(0.0, odometer.partial, 0.0)
    }
    ...
    fun `initialization clamps values above maximum`() {
        ...
        Assert.assertEquals(Odometer.MAX_TOTAL_METERS, odometer.total, 0.0)
        Assert.assertEquals(Odometer.MAX_PARTIAL_METERS, odometer.partial, 0.0)
    }
    ...
    fun `copy clamps new negative values to zero`() {
        ...
        Assert.assertEquals(100.0, updated.total, 0.0)
        Assert.assertEquals(0.0, updated.partial, 0.0)
    }
    ...
    fun `copy clamps new values above maximum`() {
        ...
        Assert.assertEquals(Odometer.MAX_TOTAL_METERS, updated.total, 0.0)
        Assert.assertEquals(50.0, updated.partial, 0.0)
    }
