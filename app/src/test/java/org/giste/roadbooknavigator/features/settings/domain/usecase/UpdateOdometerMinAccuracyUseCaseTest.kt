...
class UpdateOdometerMinAccuracyUseCaseTest {
    ...
    fun `invoke should call repository when accuracy is valid`() = runTest {
        // Given
        val accuracy = 20.0f
        coEvery { repository.setOdometerMinAccuracy(accuracy) } returns Unit
        ...
        coVerify { repository.setOdometerMinAccuracy(accuracy) }
    }
    ...
    fun `invoke should return failure when accuracy is negative`() = runTest {
        ...
        coVerify(exactly = 0) { repository.setOdometerMinAccuracy(any()) }
    }
