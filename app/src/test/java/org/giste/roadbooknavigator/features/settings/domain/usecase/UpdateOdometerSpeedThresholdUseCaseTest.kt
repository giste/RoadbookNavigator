...
class UpdateOdometerSpeedThresholdUseCaseTest {
    ...
    fun `invoke should call repository when threshold is valid`() = runTest {
        // Given
        val threshold = 0.5f
        coEvery { repository.setOdometerSpeedThreshold(threshold) } returns Unit
        ...
        coVerify { repository.setOdometerSpeedThreshold(threshold) }
    }
    ...
    fun `invoke should return failure when threshold is negative`() = runTest {
        ...
        coVerify(exactly = 0) { repository.setOdometerSpeedThreshold(any()) }
    }
