...
class JsoupRemoteMapDataSourceTest {
    ...
    fun `getRemoteMaps should parse HTML correctly`() = runTest {
        ...
        Assert.assertEquals(1, result.subFolders.size)
        Assert.assertEquals("europe", result.subFolders[0].name)

        Assert.assertEquals(1, result.maps.size)
        Assert.assertEquals("spain.map", result.maps[0].name)
        Assert.assertEquals(500L * 1024 * 1024, result.maps[0].size)
    }
}