...
class MapViewModelTest {
    ...
    fun `uiState should combine data from use cases`() = runTest {
        ...
        Assert.assertEquals(expectedMaps, state.localMaps)
        Assert.assertEquals(expectedSettings, state.settings)
        Assert.assertEquals(expectedLocation, state.currentLocation)
    }
    ...
    fun `uiState should reflect empty maps when none are downloaded`() = runTest {
        ...
        Assert.assertEquals(0, state.localMaps.size)
    }
    ...
    fun `uiState should have null location initially if use case has not emitted`() = runTest {
        ...
        Assert.assertNull(viewModel.uiState.value.currentLocation)
        Assert.assertEquals(emptyList<MapFile>(), viewModel.uiState.value.localMaps)
    }
}