...
class OdometerIntegrationTest {
    ...
    @Test
    fun `full flow should update persistence and emit new state when movement occurs`() =
        runTest(testDispatcher) {
            ...
            Assert.assertEquals(111.194, lastOdometer.total, 0.01)
    ...
    @Test
    fun `odometer should survive settings changes and keep tracking from last valid point`() =
        runTest(testDispatcher) {
            val results = mutableListOf<Odometer>()
            val job = backgroundScope.launch {
                getOdometerUseCase(settingsFlow, gpsFlow).collect { results.add(it) }
            ...
            Assert.assertEquals(333.582, results.last().total, 0.01)
