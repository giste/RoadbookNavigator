...
class SaveRoadbookSettingsUseCaseTest {
    ...
    private val useCase = SaveRoadbookSettingsUseCase(repository)

    @Test
    fun `invoke should call repository to save threshold`() = runTest {
        // Given
        val threshold = 400L
        coEvery { repository.saveShortDistanceThreshold(threshold) } returns Unit

        // When
        useCase(threshold)

        // Then
        coVerify { repository.saveShortDistanceThreshold(threshold) }
    }
}
