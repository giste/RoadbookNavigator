 ...
 */
@JvmInline
value class PollingIntervalThreshold(
    @IntRange(from = MIN, to = MAX) val milliseconds: Long
) {
    init {
    ...
    companion object {
        const val MIN: Long = 100L
        const val MAX: Long = 2000L
    }
}
 ...
 */
@JvmInline
value class MinDistanceThreshold(
    @FloatRange(from = MIN.toDouble(), to = MAX.toDouble()) val meters: Float
) {
    init {
